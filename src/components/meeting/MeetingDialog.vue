<template>
    <Dialog v-model:visible="display" :modal="true" @hide="() => emit('done')">
        ...
        <div class="mt-5 grid grid-cols-3 gap-4">
            <div>
                <label class="block mb-2 text-primary-dark-500 dark:text-primary-dark-100" for="meeting_room_id">
		        	Codigo de sala:
		        </label>
                <Dropdown
			    	id="meeting_room_id"
			    	v-model="item.meeting_room_id"
			    	...
			    	optionLabel="code_room"
			    	optionValue="id"
			    	:disabled="readonly || loading" />
            </div>
            <div>
                <label class="block mb-2 text-primary-dark-500 dark:text-primary-dark-100" for="start_meeting">
		        	Fecha de inicio:
		        </label>
		        <Calendar
		        	v-model="item.start_meeting" id="start_meeting" :model-value="item.start_meeting" :disabled="readonly || loading" name="start_meeting"
                    showTime hourFormat="12"
		        	class="shadow-sm focus:ring-indigo-700 focus:border-indigo-700 w-full sm:text-sm border-slate-300 rounded-md" />
            </div>
            <div>
                <label class="block mb-2 text-primary-dark-500 dark:text-primary-dark-100" for="finish_meeting">
		        	Fecha final:
		        </label>
		        <Calendar
		        	v-model="item.finish_meeting" id="finish_meeting" :model-value="item.finish_meeting" :disabled="readonly || loading" name="finish_meeting"
                    showTime hourFormat="12"
		        	class="shadow-sm focus:ring-indigo-700 focus:border-indigo-700 w-full sm:text-sm border-slate-300 rounded-md" />
            </div>
            <div v-if="statusAxios !== 'post'">
                <label class="block mb-2 text-primary-dark-500 dark:text-primary-dark-100" for="status_meeting">
...
const defaultData = {meeting_room_id: '', start_meeting: '', finish_meeting: '', status_meeting: 'Programado'};
